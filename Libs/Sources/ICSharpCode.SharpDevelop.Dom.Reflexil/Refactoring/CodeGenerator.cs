...
namespace ICSharpCode.SharpDevelop.Dom.Refactoring
{
	...
	public abstract class CodeGenerator
	{
        ...
        public static bool HasFlag(ModifierEnum value, ModifierEnum flag)
        {
            return (value & flag) != 0;
        }
		...
		public static Modifiers ConvertModifier(ModifierEnum modifiers, ClassFinder targetContext)
		{
            ...
            if (HasFlag(modifiers, ModifierEnum.Static))
		...
		public static AttributedNode ConvertMember(IMethod m, ClassFinder targetContext)
		{
			if (m.IsConstructor) {
				...
				return new MethodDeclaration {
                    ...
                    Body = (HasFlag(m.Modifiers, ModifierEnum.Abstract) ||HasFlag(m.Modifiers, ModifierEnum.Extern)) ? null : CreateNotImplementedBlock(),
		...
		public static PropertyDeclaration ConvertMember(IProperty p, ClassFinder targetContext)
		{
			...
			if (p.CanGet) {
                md.GetRegion = new PropertyGetRegion((HasFlag(p.Modifiers, ModifierEnum.Abstract) || HasFlag(p.Modifiers, ModifierEnum.Extern)) ? null : CreateNotImplementedBlock(), null);
				md.GetRegion.Modifier = ConvertModifier(p.GetterModifiers, null);
			}
			if (p.CanSet) {
                md.SetRegion = new PropertySetRegion((HasFlag(p.Modifiers, ModifierEnum.Abstract) || HasFlag(p.Modifiers, ModifierEnum.Extern)) ? null : CreateNotImplementedBlock(), null);
		...
		public static FieldDeclaration ConvertMember(IField f, ClassFinder targetContext)
		{
            ...
            else if (HasFlag(f.Modifiers, ModifierEnum.Fixed))
            {
		...
		public static AttributedNode ConvertClass(IClass c, ClassFinder targetContext)
		{
			if (c.ClassType == Dom.ClassType.Delegate) {
				...
				t.Children.AddRange(c.InnerClasses.Select(c2 => ConvertClass(c2, targetContext)).Cast<INode>());
