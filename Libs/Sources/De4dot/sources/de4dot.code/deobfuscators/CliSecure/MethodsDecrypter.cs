...
namespace de4dot.code.deobfuscators.CliSecure {
	...
	class MethodsDecrypter {
		...
		PeImage peImage;
		DeMono.Cecil.ModuleDefinition module;
		...
		public bool decrypt(PeImage peImage, DeMono.Cecil.ModuleDefinition module, CliSecureRtType csRtType, ref DumpedMethods dumpedMethods) {
			this.peImage = peImage;
		...
		void decryptMethodsOld(MetadataType methodDefTable, ref DumpedMethods dumpedMethods) {
			...
			for (int i = 0; i < methodDefTable.rows; i++, offset += methodDefTable.totalSize) {
				...
				var method = (DeMono.Cecil.MethodDefinition)module.LookupToken((int)dm.token);
